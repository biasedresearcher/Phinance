...
6. Sign in with the same account on each device. Automatic sync starts once local records are ready; the floating button shows progress and any conflicts.

Sync runs automatically 800 ms after a saved change, on app startup, on reconnect/focus, and every 10 seconds while the page is visible. It uses polling rather than Supabase Realtime, so no publication or database changes are needed. Keep the app open until the floating indicator says **Up to date**. Closed apps do not sync in the background; edits saved locally resume syncing on the next visit. The floating button also offers **Sync now**.

Each account has a durable last-synced snapshot and revision. One-sided changes upload/download automatically. Two independently changed snapshots pause for review; neither is overwritten. The floating panel offers an explicit choice and saves a recovery copy on the device and downloads backups of both copies first. Settings can export the last conflict backup again. There is no automatic merge of concurrent changes. A fresh empty device downloads the cloud copy; matching existing copies link automatically. Existing manual-sync revision markers are honored. Previously unlinked nonempty, differing copies require a one-time choice.

Offline changes remain in local storage. Failed uploads are retried; revision checks protect against racing writes. A download checks that local records have not changed while awaiting the write lock. Sync pauses in demo mode, for corrupt records/storage errors, and when a browser profile is associated with a different account. Use a separate private browser profile for another account.
...
Automatic-sync regression checks include independent browser profiles, offline reconnect, simultaneous edits, conflict resolution, account-change isolation, legacy revision migration and corrupt metadata. These use a mocked cloud API; production connectivity remains dependent on Supabase availability.
